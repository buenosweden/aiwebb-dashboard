...
`;
  ...
  const message = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Generera en komplett hemsida-payload som JSON. Svara ENDAST med giltig JSON, inga backticks, inga förklaringar.
...
Schema:
{
  ...
  "sections": [
    { "type": "hero", "data": { "eyebrow": "string", "headline": "string", "subheadline": "string", "primary_cta": { "label": "string", "url": "#kontakt", "style": "primary" }, "secondary_cta": { "label": "string", "url": "#om" } } },
    ...
    { "type": "faq",
cat > ~/Downloads/aiwebb-dashboard/app/api/generate/route.ts << 'EOF'
import Anthropic from "@anthropic-ai/sdk";
import { NextRequest, NextResponse } from "next/server";
import { createClient } from "@/lib/supabase-server";
import { publishToWordPress } from "@/lib/wordpress";
